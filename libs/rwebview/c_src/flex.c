        ...
        struct flex_layout *layout)
{
    ...
    // Clamp to 0: guards against floating-point underflow, NaN propagation,
    // or containers sized below their own padding.
    if (!(width  >= 0)) width  = 0;
        ...
        struct flex_layout *layout)
{
    ...
    for (unsigned int i = child_begin; i < child_end; i++) {
        ...
        // CSS Flexbox spec §9.7: a flex item's main size must not be negative.
        if (CHILD_SIZE(child) < 0.0f) {
            CHILD_SIZE(child) = 0.0f;
        }
        ...
        switch (child_align(child, item)) {
            ...
            case FLEX_ALIGN_STRETCH:
                if (isnan(align_size) || align_size == 0) {
                    // In wrap mode all-NaN-cross-size lines leave line_dim==0
                    // until the need_lines second pass.  Use align_dim as a
                    // provisional size so the recursive layout_item call
                    // receives a valid (non-NaN) dimension.
                    float effective_line = layout->line_dim > 0
                        ? layout->line_dim : layout->align_dim;
                    if (effective_line < 0) effective_line = 0;
                    CHILD_SIZE2(child) = effective_line
                        - (CHILD_MARGIN(child, left, top)
                                + CHILD_MARGIN(child, right, bottom));
                    // CSS spec: cross size must not be negative.
                    if (CHILD_SIZE2(child) < 0.0f) {
                        CHILD_SIZE2(child) = 0.0f;
                    }
                }
                // fall through
...
#undef ABSOLUTE_SIZE
        ...
        // Cross axis size defaults to the parent's inner size (padding already
        // subtracted into layout->align_dim) minus child margins.
    ...
    if (layout->need_lines && layout->lines_count > 0) {
        ...
        for (unsigned int i = 0; i < layout->lines_count; i++) {
            ...
            for (unsigned int j = line->child_begin; j < line->child_end;
                ...
                // Update cross size when:
                //   (a) it was never set (still NaN), OR
                //   (b) the item had no explicit cross dimension — the first-pass
                //       STRETCH handler may have written a provisional value based
                //       on align_dim; the need_lines second pass overrides it with
                //       the correct per-line share.
                bool had_nan_cross = isnan(
                    layout->vertical ? child->width : child->height);
                if (had_nan_cross || isnan(CHILD_SIZE2(child))) {
