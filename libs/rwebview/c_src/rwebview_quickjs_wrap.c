 ...
 *
 * Compiled via {.compile: "c_src/rwebview_quickjs_wrap.c".} in rwebview.nim.
 * The include path (-I bin/include) is supplied by rwebview.nim's {.passC.}.
 * The header resolved is bin/include/quickjs.h which is QuickJS
 * (#define QUICKJS_NG 1).
 *
 * NOTE: This file must be compiled with -std=gnu99 (set by rwebview.nim's
 *       {.passC.} pragma) because QuickJS uses GCC extensions.
 */
...
/* ── context opaque (used by rgss_quickjs.nim RGSS adaptor) ─────────────────
   Nim's generated C uses mangled type names so it cannot directly call
   JS_SetContextOpaque which expects JSContext*.  We expose a void* variant
   so the Nim importc declaration can use plain pointer parameters.          */
void rw_JS_SetContextOpaque(void *ctx, void *opaque) {
    JS_SetContextOpaque((JSContext*)ctx, opaque);
}
