...
/* SDL3 + NanoVG renderer (stb_truetype font, vector shapes) */
...
#define COL_EDGE      (SDL_Color){ 88, 102, 124, 255}
...
static void draw_slot_text(mu_Context *ctx, swu_context *swu, const char *id,
                           const char *text, int color_id, int opt) {
    swu_clip_push(swu, id);
    mu_draw_control_text(ctx, text, swu_get_rect(swu, id), color_id, opt);
    swu_clip_pop(swu);
}
...
static swu_elem_t *slot(swu_context *swu, const char *id, float w, float h) {
    ...
    swu_css_background_color(el, (SDL_Color){ 255, 255, 255, 10 });
    swu_css_border_color(el, (SDL_Color){ 88, 102, 124, 96 });
    swu_css_border_width(el, 1);
    swu_css_overflow(el, SWU_OVERFLOW_HIDDEN);
    swu_close(swu);
    return el;
}
...
static void build_layout(swu_context *swu, int win_w, int win_h) {
    ...
    swu_css_border_color(el, COL_EDGE);
    swu_css_border_width(el, 1);
    {
    ...
    swu_css_flex_grow(el, 1);
    {
        ...
        swu_css_overflow(el, SWU_OVERFLOW_AUTO);
        swu_css_background_color(el, COL_SIDEBAR);
        swu_css_border_color(el, COL_EDGE);
        swu_css_border_width(el, 1);
        {
            el = slot(swu, "SidebarLogo", NAN, 84);
            swu_css_margin_bottom(el, 8);
...
static void build_mu_widgets(swu_context *swu, mu_Context *ctx) {
    ...
    {
        if (g_logo_image != 0) {
            r_draw_image(g_logo_image, swu_get_rect(swu, "SidebarLogo"),
        ...
        if (g_show_font) {
                         ...
                         (int)g_text_color[2], 255));
            {
                ...
                swu_clip_push(swu, "FontColorPrev");
        ...
        if (g_show_tree) {
            ...
            mu_begin_panel_ex(ctx, "TreePanel", MU_OPT_NOFRAME);
        ...
        draw_slot_text(ctx, swu, "Title",
                   "SDL WEBUI \xe2\x80\x94 Phase 3b CSS/HTML API",
        ...
        set_font(ctx, R_FONT_HEADER);
        draw_slot_text(ctx, swu, "Card1Title", "Column 1 \xe2\x80\x94 Bold",
                   MU_COLOR_TEXT, 0);
        set_font(ctx, R_FONT_NORMAL);
        ...
        swu_place(swu, "Card1Body");
        {
            ...
            mu_begin_panel_ex(ctx, "Card1Panel", MU_OPT_NOFRAME);
        ...
        set_font(ctx, R_FONT_HEADER);
        draw_slot_text(ctx, swu, "Card2Title", "Column 2 \xe2\x80\x94 Italic",
                   MU_COLOR_TEXT, MU_OPT_ALIGNCENTER);
        set_font(ctx, R_FONT_NORMAL);
        ...
        swu_place(swu, "Card2Body");
        {
            ...
            set_font(ctx, fid);
            ctx->style->colors[MU_COLOR_TEXT] = body_col;
            mu_begin_panel_ex(ctx, "Card2Panel", MU_OPT_NOFRAME);
            render_text_block(ctx,
        ...
        set_font(ctx, R_FONT_HEADER);
        draw_slot_text(ctx, swu, "Card3Title", "Column 3 \xe2\x80\x94 Underline",
        ...
        swu_place(swu, "Card3Body");
        {
            ...
            set_font(ctx, fid);
            ctx->style->colors[MU_COLOR_TEXT] = body_col;
            mu_begin_panel_ex(ctx, "Card3Panel", MU_OPT_NOFRAME);
            render_text_block(ctx,
        ...
        mu_begin_panel_ex(ctx, "Console", MU_OPT_NOFRAME);
...
int main(int argc, char **argv) {
    ...
    g_logo_image = r_load_image("Logo.png");
    ...
    write_log("SDL_webui Phase 3b + NanoVG / FreeType font backend.");
