(function () {
      ...
      Window: {
        get: function () {
          return {
            ...
            capturePage: function (cb) { if (typeof cb === "function") cb(null); },
          };
        },
      },
      App: {
  ...
  // =========================================================================
  // DOMParser + XPathResult polyfill
  // Implements the subset used by the Construct2 XML plugin:
  //   DOMParser.parseFromString(str, "text/xml")
  //   xmlDoc.evaluate(xpath, contextNode, null, resultType, null)
  //   XPathResult.{STRING_TYPE, NUMBER_TYPE, ORDERED/UNORDERED_NODE_SNAPSHOT_TYPE}
  // XPath subset: absolute paths like /root/child[@attr="val"]
  // =========================================================================
  (function () {
    ...
    function makeNode(tagName, attrs) {
      return { tagName: tagName, attrs: attrs || {}, children: [], textContent: '' };
    }
    ...
    function parseXML(xmlStr) {
      ...
      while (pos < len) {
        var lt = xmlStr.indexOf('<', pos);
        // Text before next tag
        if (lt < 0) {
          if (stack.length) stack[stack.length - 1].textContent += unescXml(xmlStr.slice(pos));
          break;
        }
        if (lt > pos && stack.length)
          stack[stack.length - 1].textContent += unescXml(xmlStr.slice(pos, lt));
        pos = lt + 1;
        if (pos >= len) break;

        var ch = xmlStr[pos];
        if (ch === '?') {                         // processing instruction
          var e = xmlStr.indexOf('?>', pos); pos = e < 0 ? len : e + 2;
        } else if (ch === '!') {
          if (xmlStr.slice(pos, pos + 3) === '!--') {
            var e = xmlStr.indexOf('-->', pos); pos = e < 0 ? len : e + 3;
          } else if (xmlStr.slice(pos, pos + 8) === '![CDATA[') {
            var e = xmlStr.indexOf(']]>', pos + 8);
            var cdata = e < 0 ? xmlStr.slice(pos + 8) : xmlStr.slice(pos + 8, e);
            if (stack.length) stack[stack.length - 1].textContent += cdata;
            pos = e < 0 ? len : e + 3;
          } else {
            var e = xmlStr.indexOf('>', pos); pos = e < 0 ? len : e + 1;
          }
          ...
          var e = xmlStr.indexOf('>', pos);
          if (e < 0) { pos = len; break; }
          var tagStr = xmlStr.slice(pos, e);
          var self = tagStr[tagStr.length - 1] === '/';
          if (self) tagStr = tagStr.slice(0, -1);
          var sp = tagStr.search(/\s/);
          var tagName = sp < 0 ? tagStr : tagStr.slice(0, sp);
          var node = makeNode(tagName, parseAttrs(sp < 0 ? '' : tagStr.slice(sp)));
          pos = e + 1;
          if (self) {
            if (stack.length) stack[stack.length - 1].children.push(node);
            else root = node;
      ...
      while (stack.length > 1) { var n = stack.pop(); stack[stack.length-1].children.push(n); }
      if (stack.length === 1) root = stack[0];
      return root;
    }

    // ── XPath evaluator ─────────────────────────────────────────────────
    // Supports: /a/b[@attr="val"], /a/b, /a/b/c[@attr="val"], /a/b[1]
    function xpathEval(xpath, startNode, resultType) {
      if (!xpath || !startNode) return buildResult(resultType, []);

      // Strip leading slashes — startNode is already the doc-root wrapper
      var path = xpath.replace(/^\/+/, '');

      // Split path into steps, respecting [...] predicates
      var steps = [];
      var i = 0, cur = '';
      while (i < path.length) {
        var c = path[i];
        if (c === '[') {
          var cl = path.indexOf(']', i);
          cur += (cl < 0 ? path.slice(i) : path.slice(i, cl + 1));
          i = cl < 0 ? path.length : cl + 1;
        } else if (c === '/') {
      ...
      for (var si = 0; si < steps.length; si++) {
        var step = steps[si];
        var bi = step.indexOf('[');
        var tagName = bi < 0 ? step : step.slice(0, bi);
        var pred = bi < 0 ? null : step.slice(bi + 1, step.lastIndexOf(']'));
        var predAttr = null, predVal = null, predPos = -1;
        if (pred) {
          var pm = pred.match(/^@([\w:.-]+)\s*=\s*["']([^"']*)["']$/);
          if (pm) { predAttr = pm[1]; predVal = pm[2]; }
          else if (/^\d+$/.test(pred.trim())) predPos = parseInt(pred) - 1;
        }
        var next = [];
        for (var ni = 0; ni < nodes.length; ni++) {
          var children = nodes[ni].children || [];
          var posIdx = 0;
          for (var ci = 0; ci < children.length; ci++) {
            var child = children[ci];
            if (tagName === '*' || child.tagName === tagName) {
              if (predAttr !== null) {
                if (child.attrs && child.attrs[predAttr] === predVal) next.push(child);
              } else if (predPos >= 0) {
                if (posIdx === predPos) next.push(child);
                posIdx++;
              } else {
                next.push(child);
              }
      ...
      return buildResult(resultType, nodes);
    }

    function buildResult(resultType, nodes) {
      var XPR = window.XPathResult || {};
      var NUMBER_TYPE = XPR.NUMBER_TYPE !== undefined ? XPR.NUMBER_TYPE : 1;
      var STRING_TYPE = XPR.STRING_TYPE !== undefined ? XPR.STRING_TYPE : 2;
      var snap = nodes.slice();
      var str0 = snap.length > 0 ? snap[0].textContent : '';
      var num0 = snap.length > 0 ? (parseFloat(snap[0].textContent) || 0) : 0;
      if (resultType === NUMBER_TYPE)
        return { numberValue: num0, snapshotLength: 0, snapshotItem: function() { return null; } };
      if (resultType === STRING_TYPE)
        return { stringValue: str0, snapshotLength: 0, snapshotItem: function() { return null; } };
      // Snapshot (ORDERED / UNORDERED)
      return {
        snapshotLength: snap.length,
        snapshotItem: function(i) { return snap[i] || null; },
        stringValue: str0,
        numberValue: num0
      };
    }
    ...
    // ── XMLDocument wrapper ──────────────────────────────────────────────
    function XMLDocument(rootEl) {
      this.documentElement = rootEl;
      // Virtual document node so absolute XPaths (/root/...) traverse correctly
      this._docRoot = { tagName: '#document', attrs: {}, children: rootEl ? [rootEl] : [], textContent: '' };
    }
    XMLDocument.prototype.evaluate = function(xpath, contextNode, nsResolver, resultType, result) {
      // Absolute paths always start from document root regardless of contextNode
      var start = (xpath && xpath.charAt(0) === '/') ? this._docRoot : (contextNode || this._docRoot);
      return xpathEval(xpath, start, resultType);
    };

    // ── DOMParser ───────────────────────────────────────────────────────
    window.DOMParser = function() {};
    window.DOMParser.prototype.parseFromString = function(str /*, type */) {
      return new XMLDocument(parseXML(str));
    };
